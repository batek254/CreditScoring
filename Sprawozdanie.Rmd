...
```{r}
for (f in c("pmm","midastouch","sample","cart","rf")){
...
uzupelnianie_dane = c(set_knn_test,set_mf_test,set_mice_test)
...
```{r, echo=FALSE}
feature_eng_set = read.csv('C:/Users/User/Desktop/train.csv')
   ...
   set.seed(123)
    ...
    folds <- cut(seq(1,nrow(set)),breaks=6,labels=FALSE)
     for(j in 1:6){
      ...
      #Test
      set.seed(123)
      model_rf<-randomForest(Loan_Status~.,trainset,ntree=1000)
      
      #Sprawdzanie Accuracy
      pred_rf<-predict(model_rf,testset)
      acc_rf<-1 - accuracy(round(pred_rf),testset$Loan_Status)[,1]
...
wynik_wyboruZbiorow = data.frame("Nazwa metody" = rev(nazwa_metody[zbioryDoBadania]), "Atrybuty" = rev(unlist(nazwa_atrybutow)[zbioryDoBadania]), "ACC" = rev(accZbiorow[zbioryDoBadania,]))
...
wyniki_df<-data.frame("metoda"=rep(0,1),"wynik"=rep(0,1), "wynik_treningowy" = rep(0,1))
   ...
   set.seed(123)
    ...
    #Wybranie 10 najlepszych zbior?w
zbioryBazowe = tail(order(accZbiorow), 3)

#Lista ostatecznie wybranych do badania zbiorow
zbioryBazowe_lita = list()
for(i in 1:3) {
  zbioryBazowe_lita[[i]] = noweZbiory[[zbioryBazowe_lita[i]]]
}
accZbiorow[zbioryBazowe,]
nazw = c("NA - remove", "KNN - k=6", "CreditHist - k = 6")
frame_func(data.frame("Metoda"= rev(nazw[zbioryBazowe]), "ACC" = rev(accZbiorow[zbioryBazowe,])))
```
...
```{r}
#GBM
cv_pred <- function(n_times,trainset,testset,method,arg_list=NULL)
{
  acc_list = list()
  ...
  for(i in 1:n_times){
      ...
      wynik = loan_predict(trainset,testset,method,arg_list=NULL)
      acc = acc + wynik[[2]]
  }
  acc_list[[i]] = acc/n_times
}
}
